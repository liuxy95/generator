...
'fma.float.mad':['fma.rn.ftz',3,'f32'],
'mad.float.mad':['mad.rn',3,'f32'],
...
d_mad_list=['mad']
b_alu_list=['not','or','and','popc','xor','shl']
	...
	def genKernel(self,file_stream):
		...
		file_stream.write(
			'\tasm volatile(".reg .u32	%rd<'+str(self.d_alu+self.d_mad+1)+'>;\\n\\\n'#int regs   Zhibin changes. +1 at the end
				...
				'\tmov .f32	%ropf3,'+str(random.uniform(1,100))+';");\n\n'											#3td float op regs
		)
		...
		file_stream.write(
		"\t//get the loop number\n"
		"\tasm volatile(\"ld.param.u32   %loop, [_Z6kernelPfS_S_PiS0_S0_i_param_6];\");//loop\n\n"
		)
		#file_stream.flush();
	...
	def genParam(self,file_stream):
		...
		for t in tt:
			for i in tmp:
				...
				file_stream.write(
					'//'+t+i+' data\n'
					'\n\tasm volatile("ld.param.u64	%rp'+t+i+', [_Z6kernelPfS_S_PiS0_S0_i_param_'+st+'];\\n\\\n'
